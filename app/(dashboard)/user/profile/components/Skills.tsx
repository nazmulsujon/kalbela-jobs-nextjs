...
const Skills = () => {
      ...
      const { data: skills = [], isLoading, error } = useQuery({
            ...
            queryFn: async () => {
                  if (!user?._id) return [];
                  const res = await fetch(`${process.env.NEXT_APP_BASE_URL}/api/v1/user/get-skills?user_id=${user._id}`);
                  if (!res.ok) {
                  ...
                  return data?.data?.map((skill: string) => ({ value: skill, label: skill }));
            },
            enabled: !!user?._id,
      });
                        ...
                        <CardContent className="space-y-4">
                              <div className="flex flex-wrap gap-2">
                                    {visibleSkills.map((skill: Skill) => (
                                          <Badge key={skill.value} className="rounded" variant="secondary">{skill.label}</Badge>
                                    ))}
                                    {hiddenSkillsCount > 0 && (
                                          <Badge variant="secondary" className="cursor-pointer">+{hiddenSkillsCount}</Badge>
                                    )}
                              </div>
                              <Button variant="outline" className="gap-2" onClick={() => {
                                    ...
                                    {visibleSkills.length ? <Pencil className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
